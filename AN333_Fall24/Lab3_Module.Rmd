...
As you can see on first glance, there are *potentially* two large linkage blocks in the *UCP1* region of the YRI population: one downstream (near the beginning) of the region (remember, *UCP1* is on the reverse strand, so lower position numbers are downstream), and one upstream. It also looks like there may be a small region of linkage in the middle.

There are a few things we can do to figure out exactly where these linkage regions are...

Maybe the easiest is a tool in the {gpart} package for statistically pulling out the positions of independent linkage blocks:
...
Although the other two linkage blocks further upstream from the gene are not annotated on *EnsEMBL*, the loci from the Cha *et al.* paper give us a clue regarding what these linkage blocks might represent: each appear to contain the two regulatory loci Cha *et al.* identified as having a large impact on abdominal obesity in Koren women, suggesting that these may also be either full regulatory regions for *UCP1* that have not yet been identified as such, or that there may be LD around these two SNPs because of selection based on their regulatory effects on *UCP1* function.
