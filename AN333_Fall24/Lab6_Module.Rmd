...
## A Brief Digression from *COVID-19* for Quantitative Genetics
...
### [Homework for Lab 6: DUE Friday, November 22nd](https://fuzzyatelin.github.io/AN333_Fall18/Lab6_Homework.html)
...
The first thing we need to do is log in to the *SCC*, and then navigate to your personal project folder (where `username` is your personal project directory):
```{bash,eval=F, class.source="sccCode"}
cd /projectnb/anth333/username/
```

Once there, create a new directory called `vervet`:
```{bash,eval=F, class.source="sccCode"}
mkdir vervet
```

Now, navigate to a directory called `/project/anth333/vervet`:
```{bash,eval=F, class.source="sccCode"}
cd /project/anth333/vervet/
```

Once there, copy both the files from the `vervet` directory to the new `vervet` directory you created in your personal project directory. These files include the wild vervet pedigree (`vervped.csv`), and the population dataset (`vervdat.csv`) on BW, BMI and the dietary covariate. You can use the following code, as an example (in this case, replace 'filename' with the name of the file, and 'BUID' with *your* BUID, so that the file will be copied to your named folder):

```{bash,eval=F, class.source="sccCode"}
cp /project/anth333/vervet/filename /projectnb/anth333/BUID/filename
```

Finally, navigate back to your new `vervet` directory in your personal project directory at `/projectnb/anth333/username/vervet/` to make sure that the file copied properly.

Before continuing, let's take a look at the vervet data we've copied to our directories. Use the `less` command to take a look at the pedigree file, `vervped.csv` (remember, you can press 'q' at any time to get back to your prompt):
