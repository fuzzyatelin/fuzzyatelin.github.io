---
title: "Module 4 - A (Very Brief) Introduction to Phylogenetics"
...
<li>[Klingenspor M, et al. 2008. An ancient look at UCP1. *J Biochimica et Biophysica Acta - Bioenergetics* 92(6): 637-641.](https://www.sciencedirect.com/science/article/pii/S0005272808000480)</li>
...
Log in to the *SCC On Demand* and bring up the *R Studio* window [like we did in previous labs](https://fuzzyatelin.github.io/AN333_Fall24/Lab2_Module.html).
...
Before the next step, we need to actually count how many SNPs we have in our VCF file. To do that, we'll use a program called {bcftools}, which we can access on the *SCC* from our Terminal (which also requires loading a specific version of {htslib}:
```{bash, eval = F, echo = T, class.source="sccCode"}
module load htslib/1.16
module load bcftools/1.16
```

Once you're in the working directory containing the VCF file you want to summarize, enter the following code:
```{bash, eval = F, echo = T, class.source="sccCode"}
bcftools stats /projectnb/anth333/caschmit/UCP1_YRI_REGION.vcf
```

In your output, you'll see a lot of details... the one piece of information you'll need is near the top of your output, `number of records`. For YRI, it's `150`. If you look further down, you'll see that these records are divided into `number of SNPs` (117), and `number of indels` (33). That may be important later, but for now we just need the `number of records`.

Onc eyou have that, we're ready to head back to *R Studio* and import our VCF. In your `read.vcfR` call, enter the number from `number of records` for *your* VCF file into the `nrows` argument, as below:
```{r}
#read in data
YRI <- read.vcfR("UCP1_YRI_REGION.vcf",verbose=T,nrows=150)
...
Notice that the output messages tell us that the DNAbin format is ignoring all *indel* variants. This is important to our genetic distance estimates because we need our sequences to be identical in length so that nucleotide positions can be read as single point mutations.<br><br>
...
As you can see from the example, YRI is a fairly molecularly diverse population in the context of *UCP1*, and there are many different *nodes* on the tree. To interpret this tree, think of every node (or splitting/branching point) as a point at which two sub-populations differ by at least one variant. Everyone to the right of a node along one particular branch shares the same difference. The *length* of the line from left-to-right corresponds to the number of base-pair differences in the sequence (this means that longer lines from left-to-right imply more changes in the sequence, or more mutations; if we assume a constant mutations rate, this also implies more time).

We can also make this tree a bit more aesthetically pleasing using the {ggtree} package. To download {ggtree}, try this (if it asks you to update packages, if you're in a rush you can say `n`):
```{r, eval=F}
...
There are many kinds of trees we can make in {ggtree}, here are just a few, some more readable than others:
```{r,message=F,fig.width=15,fig.height=15}
library(ggtree)
ggtree(YRItree,layout='circular') + geom_tiplab(aes(angle=angle), color='blue')
ggtree(YRItree, branch.length='none', layout='circular') + geom_tiplab(aes(angle=angle), color='blue') 
ggtree(YRItree, layout="daylight") + geom_tiplab(aes(angle=angle), color='blue')
```

In most of these trees, our branch lengths represent the number of differences... as you can see, in many tree layouts that can make some parts of our tree unreadable (particularly where there are many haplotypes that are identical or very similar to each other, as they will be bunched closely together on the tree). In the second of our {ggtree} examples, I've set the `branch.length` setting to `none`. This makes the tree much more readable, and we can get a clear view of the pairwise relationships of all of our haplotypes, but unfortunately it means that the branch lengths are *not* representative of genetic distances and how genetically different each sequence might be. 
...
In the next part of the module, we'll make two more trees of populations from the same larger geographic region as your study population, and compare the tree from *your* population to these other two trees. In this way, we'll be able to qualitatively compare the amount of diversity in *your* population to the diversity in populations of the same region. Refer to [Module 1](https://fuzzyatelin.github.io/AN333_Fall24/Lab1_Module.html) for population names and regions and pick TWO other populations that are from the same region (i.e., Africa, Europe, East Asia, etc.) as your study population. 
<br>
<br>
The first step for this part of the lab will be to download the *UCP1* data from these other two populations in your region. Instead of downloading more data from *EnsEMBL*, you can get the data from the `/projectnb/anth333/` directory called "*sampleVCF*". This directory contains downloads of *UCP1* data from all of the 1000 Genomes populations. This process will be similar to what we did in [Module 1](https://fuzzyatelin.github.io/AN333_Fall24/Lab1_Module.html) to get our data in to our *SCC* folders. Just in case, here's how you would copy this files from the "*sampleVCF*" folder to your working directory (make sure that the `filename` is the same for both sections of this code... i.e., `UCP1_CLM.vcf` from the `sampleVCF` folder is being copied as `UCP1_CLM.vcf` to your `username` folder) :

```{bash, eval = F, echo = T, class.source="sccCode"}
cp /projectnb/anth333/sampleVCF/filename.vcf /project/anth333/username/filename.vcf
```

In the above instructions, `filename` should be replaced by the three-letter acronym for the population whose data you'd like to download, and `username` should be replaced by your BUID-named directory in the *SCC* project space.<br><br>

Now you have the data for two new populations in your working directory!

The next step is to merge your three VCF files, which can be accomplished using a combination of *vcftools*, *htslib* and another package called *bcftools*:
```{bash, eval = F, echo = T, class.source="sccCode"}
...
#then compress your three VCF files:
bgzip -ci UCP1_YRI_REGION.vcf > UCP1_YRI_REGION.vcf.gz
bgzip -ci UCP1_LWK_REGION.vcf > UCP1_LWK_REGION.vcf.gz
bgzip -ci UCP1_ASW_REGION.vcf > UCP1_ASW_REGION.vcf.gz

#then merge them:
bcftools merge -0 UCP1_YRI_REGION.vcf.gz UCP1_LWK_REGION.vcf.gz UCP1_ASW_REGION.vcf.gz | bgzip -c > UCP1_AFR_REGION.vcf.gz
```
You can now return to your *R Studio* screen, where you should see the new files in your directory in the bottom right screen (click on the "Files" tab). We will now use these files to create trees that we can compare to our original population. 
<br><br>
...
```{r}
#Tree 1
LWK <- read.vcfR("UCP1_LWK_REGION.vcf")
...
#Tree 2
ASW <- read.vcfR("UCP1_ASW_REGION.vcf")
...
```
<br>
Finally, plot all the trees on the same axis. It will probably take some time to render the plots because there are a *lot* of tips (each representing an individual haplotype in your populations) on the tree: 
...
## Part 5: A Unified Phylogeny

**UNDER CONSTRUCTION**

Unfortuantely, this section of the module appears to be broken. We'll hopefully be able to run this in the next iteration of the course!

## Part 6: What Do Your Results Mean? Discuss with a partner from class: 
