...
Here's some easy code to filter out JUST those SNPs that are significant:
```{r}
library(tidyverse)
HWE.sig <-
  HWE %>%
  as_tibble(HWE) %>%
  add_rownames(var = "snp") %>%
  rename(chi2 = 'chi^2', pval = 'Pr(chi^2 >)') %>%
  filter(pval <= 0.05)
HWE.sig
```

Record the SNP ID numbers that have significant P-values, because they will be important for what we do next. 
<br>
<br>
