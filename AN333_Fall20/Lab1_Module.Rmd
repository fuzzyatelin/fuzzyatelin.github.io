...
* First, we can download the data. Remember, mine is YRI, so I'll use that code. Where you see 'YRI' in my code, insert the three-letter code of *your* population. Use the following command, making sure that your positions are correct for ACE2 (X:15561033-15602148), and that the chromosome designation is correct (e.g., `ALL.chrX...`):
```{bash, eval = FALSE, echo = T, class.source="sccCode"}
tabix -h ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/supporting/GRCh38_positions/ALL.chrX_GRCh38.genotypes.20170504.vcf.gz X:15561033-15602148 > ACE2_YRI_all.vcf
```
...
```{bash, eval = FALSE, echo = T, class.source="sccCode"}
vcftools --vcf ACE2_YRI_all.vcf --keep YRI.samples.list --recode --out ACE2_YRI
```
