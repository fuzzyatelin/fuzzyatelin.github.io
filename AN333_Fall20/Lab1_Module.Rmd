---
title: '**AN/BI 333/733 Module 1**<br>Accessing Human Candidate Gene Region Data – *ACE2* and *TMPRSS2*'
...
```{bash, eval = FALSE, echo = T, class.source="sccCode"}
tabix -h ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/supporting/GRCh38_positions/ALL.chr4_GRCh38.genotypes.20170504.vcf.gz X:15561033-15602148 > YRI_ACE2_all.vcf
```
...
```{bash, eval = FALSE, echo = T, class.source="sccCode"}
vcftools --vcf YRI_ACE2_all.vcf --keep YRI.samples.list --recode --out YRI_ACE2
```

* My *new* VCF file is called ***YRI_ACE2.recode.vcf***. Again, yours should be similar but with *your* population three-letter name. Rename this, as follows:
```{bash, eval = FALSE, echo = T, class.source="sccCode"}
mv YRI_ACE2.recode.vcf YRI_ACE2.vcf
```

* Great! You now have your *ACE2* data file for just your population, which should be named ***YRI_ACE2.vcf*** (but with your population name)! If you take a look at the VCF file and scroll, you should see a list of IDs that match your samples list, meaning that the only genotypes included are those from your sample population:
```{bash, eval = FALSE, echo = T, class.source="sccCode"}
less YRI_ACE2.vcf
```
...
# Congratulations, you now have your genetic data for *ACE2*!
