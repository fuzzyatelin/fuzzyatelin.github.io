...
The SMAGL has both a wet bench lab and a computational dry lab at our space in the Stone Science Building (STO), Room 251. See the section on [COVID-19 Protocols and Emergency Preparedness](https://fuzzyatelin.github.io/smagl_student_guide/smagl_covid.html) for policies on access to SMAGL facilities that are put into place during pandemic lockdowns.

Access to the SMAGL is regulated by swipe card access mediated through your BU Terrier Card. If you require access to the Dry or Wet Lab, please let Prof. Schmitt know. With his permission, Lab Coordinator Dr. Christian Gagnon can give access on a semester-by-semester basis.

**All individuals who require access to the SMAGL wet bench space in STO 251 *must* [complete the required trainings](https://fuzzyatelin.github.io/smagl_student_guide/smagl_trainings.html) *before* entering!** 

## SMAGL Dry Lab

The dry lab has four (4) workstations, and no special restrictions regarding food and drink or proper apparel. Priority of access to these workstations generally goes to SMAGL PRAs first, then to Ph.D. students conducting dissertation research, then to other graduate students, then to undergraduate students. *Students not affiliated with the SMAGL should not be using these workstations without permission of a SMAGL PI*.
...
- **Workstation B** (on the interior wall immediately next to A) is a flexible workstation with a monitor and large external drive that SMAGL students may plug their laptops in to for ease of work. There is a sign-up list for this station on the [Schmitt Lab Drive](https://fuzzyatelin.github.io/smagl_student_guide/smagl_online.html).
- **Workstation C** (on the interior wall across from Workstation B) is dedicated to Schmitt Lab use for bioinformatics or high-computational capacity research. This is the only workstation that currently has the hardware, memory, and software necessary for these kinds of analyses in the Dry Lab. Research/work that requires this equipment receives priority over all other uses. **Please contact Jimmy Erkens regarding use of this workstation**.
- **Workstation D** (nearest the hallway entrance) is a flexible workstation with a monitor and large external drive that SMAGL students may plug their laptops in to for ease of work. There is a sign-up list for this station on the [Schmitt Lab Drive](https://fuzzyatelin.github.io/smagl_student_guide/smagl_online.html).
...
-	The **PCR 1 Workstation** is opposite the Sequencing and Quantitation Workstations. This station is dedicated to the amplification of DNA using PCR and should be used for no other purposes.
-	The **PCR 2 Workstation** is behind the PCR 1 Workstation. This station is dedicated to the amplification of DNA using PCR and should be used for no other purposes.
...
The other two (2) workstations (E and F; each on the backside of the Freezer Workstation), are flexible use dedicated for use by Schmitt Lab researchers conducting bioinformatics research. If you want access to one of these workstations, please contact Prof. Schmitt.

Swipe card access is available for the STO 251 space and must be renewed each semester. If you require swipe card access to use STO 251 spaces and resources, please discuss with Prof. Schmitt and Lab Coordinator Dr. Christian Gagnon.

### SMAGL Scheduling

We'll be very busy in the SMAGL wet lab this semester! We have several projects ongoing, and few new ones, including:

* DNA metabarcoding for *Lagothrix* (Schmitt/Pain/Bronstein)
* Microbiome in *Lagothrix* (Schmitt/Pain/Martin)
* Mitogenomics in *Lagothrix* (Schmitt/Ndukwe)
* Urinary DNA in *Pongo* (Knott/Scott)
* hDNA mitogenomics from *Chlorocebus* museum specimens (Schmitt/Erkens)
* airDNA metabarcoding from Perú (Schmitt/Pain/Finnerty)
...
**Do not** use the following equipent without explicit clearance/training from Prof. Schmitt, a PRA (Dr. Pain or Dr. Scott), or senior Ph.D. students Jess Martin or Jimmy Erkens:

* BioSafety Cabinet (BSC)
* Autoclave (Gel Workstation)
* Centrifuges
* Hula Shaker (Extraction Station)
* MinION Mk1D sequencer (Sequencing Station)
...
The following Schmitt Lab graduate students have dedicated space in the Biological Anthropology Graduate Student Space for the Fall 2026 semester:

-	**Soumalya Ghorui**
-	**Jimmy Erkens**
-	**Jess Martin**

Please keep in mind that Schmitt Lab use of this space is provisional and will be re-assessed for each semester. Those currently with space here may not continue in this space given the needs of other students with priority of access. Those of you with current assigned seats must contact Lab Coordinator Dr. Christian Gagnon to get swipe card access specific to the Biological Anthropology Graduate Student Space.
...
Access to the Suite 247 area is open from 8am to 6pm M-F, and after-hours swipe card access is available upon request for those actively conducting research in the SMAGL (please discuss with Prof. Schmitt and/or Lab Coordinator Dr. Christian Gagnon).
...
The BU LGBTQIA+ Student Resource Center lists several [gender-neutral/all-gender bathrooms](https://www.bu.edu/lgbtqia-student-center/community/on-campus-resources/) in our building. The closest to the SMAGL is on the first floor of the CAS building. Walk down the hall and around the corner to the Commonwealth Ave side of the building, and go down one flight of stairs, it’s to the immediate left of the stair exit on the first floor. There is also a men’s bathroom in the same hallway as the SMAGL on the lab-side of the doors to CAS, and a women’s bathroom on the CAS-side of the same doors.

# Chemical Anthropology Lab

Next door to the SMAGL in STO 249 is the **Chemical Anthropology Lab**. This lab is open for use by anyone in Anthropology given access, and is primarily intended for use with hazardous chemicals.
...
You can ask Lab Coordinator Dr. Christian Gagnon for swipe access, if required.
