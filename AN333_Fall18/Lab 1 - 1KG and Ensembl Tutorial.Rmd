...
* Enter this code in your prompt to load *tabix*:<br>

> module load tabix

* First, let's download the ID Panel from 1000 Genomes. This is a list of IDs associated with each genome, including their population of origin:

> wget ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/integrated_call_samples_v3.20130502.ALL.panel

* Now, we need to make a list of IDs just from our population of interest, so we can tell *tabix* to only keep those individuals. Remember, mine is YRI, so I'll use that code. Where you see 'YRI' in my code, insert the three-letter code of *your* population:

> grep YRI integrated_call_samples_v3.20130502.ALL.panel | cut -f1 > YRI.samples.list

* Let's now load *vcftools*, which has the *vcf-subset* tool we need to use to filter out our population:

> module load vcftools

* Now, we can download the data and filter it *at the same time*! Use the following command:

> tabix -h ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/supporting/GRCh38_positions/ALL.chr4_GRCh38.genotypes.20170504.vcf.gz 4:140559434-140568805 |perl vcf-subset YRI.samples.list | bgzip -c YRI.vcf.gz

* Notice the syntax in that command? It's basically:

> tabix -h [data URL] [region lookup] | perl vcf-subset [POP].samples.list | bgzip -c [new file name].vcf.gz

* Let's also download the smaller region:

> tabix -h ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/supporting/GRCh38_positions/ALL.chr4_GRCh38.genotypes.20170504.vcf.gz 4:140560308-140564106 | bgzip -c YRIsmall.vcf.gz
