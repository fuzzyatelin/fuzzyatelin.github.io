...
* Let's now load *vcftools*, which has the *--keep* option we need to use to filter out our population:
```{r, eval = FALSE}
...
* Great! You now have your *UCP1* data file for just your population, which should be named ***YRI.vcf*** (but with your population name)!
...
* Use the same post-download processing to subset this smaller regional dataset in the same way as the larger, name the final file ***YRIsmall.vcf*** (with your own population name, of course), and you're finished!
<br>
<br> 
