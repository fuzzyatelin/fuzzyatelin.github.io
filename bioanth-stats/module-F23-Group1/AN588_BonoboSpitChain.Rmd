...
Let's start by loading our data from bonobos. There are two datasets from the same zoo-based bonobo populations at Apenheul Primate Park in the Netherlands, and La Vallée des Singes in France. One dataset describes GG rubbing networks and the other describes grooming networks in these populations.
...
Seeing this output is fine... but not as intuitive as seeing something like Alice Pieszecki's Chart. For a more intuitive figure, we can visualize social networks by creating network objects with {statnet}:
```{r visualize}
par(mfrow = c(1, 2))
...
**Can we manipulate the plots so that the node colors are different based on relevant variables like sex? Can we benefit from showing edge weights in the spider monkey plots as well?**
