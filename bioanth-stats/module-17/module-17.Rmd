...
dat <- data.frame(
  index = c(1),
  labels = c("gre"),
  OR = c(exp(glm$coefficients[2])),
  LL = c((exp(glm$coefficients[2]-CI[2,1]))),
  UL = c((exp(glm$coefficients[2]+CI[2,2]))))
dat
...
plot1 <- ggplot(dat, aes(y = index, x = OR)) +
  ...
  scale_y_continuous(name = "", breaks=1, labels = dat$label, trans = "reverse") +
...
As always, this can also be more simply (and elegantly) done using a dedicated package; in this case, the {[ggstats](https://larmarange.github.io/ggstats/articles/ggcoef_model.html)} package:
```{r,fig.height=2,fig.width=5}
library(ggstats)
