---
title: "Factor Analysis and Principle Component Module"
...
In this module, we will describe both factor analysis (FA) and principal component analysis (PCA), while also differentiating between the two. We will demonstrate the usefulness of both of these statistical models using a functioning dataset. Also, we will use multiple codes/ packages that perform factor analysis and principal components. Finally, we will show previous studies that have used either method.  
...
<br>* Talkativeness
<br>* Frequency of Penetrative Sex
<br>* Willingness to pay for a date (money does buy love?)
<br>* Desire to see one another
<br>* Luvin' & Leavin'
<br>* Deriving pain from talk of marriage
<br>* Sassiness
<br>* Laughing
<br>* Sweaty Pits
<br>* Dry Throat
<br>* Frequency of Making Out
<br>* Enjoying that marriage talk
<br>* Frequency of Annoying, Meaningless Texts
<br>* Spontaneous Acts of Kindness (Romantic Gestures?)
<br>* Number of Dates (<i>how</i> romantic)
<br>* Eye Contact
<br>* Engagement in Conversations
<br>* Horniness
<br>* Length of time spent looking at your counterpart's parts 
<br>* Unnecessary PDA
<br>* Huggin' & Luvin'
<br>* Eye Fluttering
<br>* Commitment
<br>* Frequency of Oral Sex
<br>* Genuine Concern
<br>* Artificial Concern (with the expectation of sex)
<br>* Frequency of Sexting
<br>* Butt slapping
...
* Factor analysis requires a large sample size
* 100-200 acceptable with well determined factors
* At least 300 cases with smaller communalities
* Well over 500 with lower communalities and larger numbers of weakly determined factor
* Random data can give factors
...
* May not be able to pool results of samples because if the samples vary on an important criterion, such as socio-economic status, they may have a different underlying factor structure.
* The answers you get depend entirely on the questions you ask
* The factors that appear can only come from the questions that you ask
* Selecting a good set of questions to get at all of the latent variables is difficult
* Logically in order to completely reveal a process underlying an area of research all relevant factors must be included. Therefore, if you miss a relevant factor when you are constructing a measure such as a survey then your factor structure and the results you produce are meaningless as you are missing a critical component essential to the factor structure. Essentially failure to measure factor may distort the apparent relationship between factors.
...
* Ideally want to have multivariate normality means that all linear combinations of the variables are normal
...
Let's load in data and get started:
...
Based on an analysis of the columns (see above) it appears that the data is missing at random and we can proceed with replacing the missing data with the mean of the given variable using the package <b>mice</b>.
...
<b>Mahalanobis Distance:</b> one of the methods used in detecting MVOs is the Mahalanobis statistic (usually referred to as the Mahalanobis Distance). The Mahalanobis Distance measures how many standard deviations a point of prediction (P) is from a distribution (D). If P overlaps with the mean of D, the distance comes up as zero. Its nature is unitless, scale-invariant, and takes into account the correlations in the set, therefore it comes in handy while doing factor analysis.

We can screen for multivariate outliers using nomiss and mahalanbis. Multivariate outliers are data that are unexpected. So, in this case, it would be responses from participants that are unusual compared to other participants. 
...
From the chunk above, you can see that there are 34 individuals that are considered 'outliers', while the bulk of our participants have 'expected' responses (N=237). 
...
In the table above, we removed 34 individuals. For example, you can see that individual #7 was removed (the row numbers jump from 6 to 8). 
...
The <b>Kieser Meyer Oken (KMO) test</b> assesses sampling adequacy. To boil it down- do you have enough samples? For this example, our sample size is N=237. Results from this test range between 0 and 1 with larger values representing more adequate smaples (We want 1!We want 1!We want 1!).
...
<br>You can assess all three eigenvalue criteria at the same time via the <b>fa. parallel()</b>
...
```
On our Scree Plot, the black horizontal line is drawn at 1. This represents an eigenvalue= 1. We are going to count the number of factors (little blue triangles) and include that factor number in our analysis. 

Hard to see, but there are 7 little blue triangles with an eigenvalue greater than 1. This is based off of the Kaiser-Harris Criterion. 
...
In the output above, you look at each row, which represents 1 of our 79 variables. In each row, you want to see at least one absolute factor loading of 0.30 or greater. If a row has this qualification, then you keep it. Otherwise, eliminate it from the analysis. 

Round 2! Conduct second factor anlaysis with oblimin roation and fm being maximum likelihood. Notice that we are removing certain rows from our analysis. 
...
sd(noout$f7)
```
Now, the exciting part (drum roll please): our factor plot
...
<b>Brainz Example (Paleoanthropology)</b>
...
Strait, D. S. (2001). <i>Integration, phylogeny, and the hominid cranial base.</i> American Journal of Physical Anthropology, 114: 273–297. doi:10.1002/ajpa.1041.
<br>
<br><b> Orangutan Example </b>
...
<br>
<br>For one of their many analyses, they ran a principle components analysis on traits related to the genetic diversity seen across different populations of orangutans. Take a look at the plot of PCA1 and PCA2: 
<br>
<img src= "PCAorangutan.PNG">
<br>
<br> Note anything fishy? The answer is yes, yes you do. PCA1 is awesome! We like PCA1- it accounts for 34.2% of the variance in our samples (really I mean species). PCA2 only accounts for a tiny 3.6% of variance, BUT it makes tapanuli orangutans look like a totally different species from Sumatran orangutans (<i>P. abelii</i>) and Bornean orangutans (<i>P. pygmaeus</i>).
<br>
